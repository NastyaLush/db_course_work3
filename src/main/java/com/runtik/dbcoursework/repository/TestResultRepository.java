...
public class TestResultRepository {
    ...
    public Page<List<TestResultSelectDTO>> getTestResults(
            Pageable pageable,
            List<SortField<?>> sortFields,
            Condition condition
    ) {
        try (SelectJoinStep<Record2<String, String>> table =
                     dslContext.select(
                                     Tables.PERSON.PERSON_NAME,
                                     Tables.FRACTION.FRACTION_NAME)
                             .from(Tables.TEST_RESULT)
        ) {
            SelectConditionStep<Record2<String, String>> selectConditionStep =
                    table.join(Tables.FRACTION)
